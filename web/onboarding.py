...
from web.zapi import zapi as _zapi_client
...
async def _iniciar_onboarding(phone: str):
    await _zapi_client.enviar(phone,
        ...
        "com feriados forenses de *2.091 localidades* em todo o Brasil.\n\n"
...
async def _handle_onboarding(phone, texto, step, dados):
    if step == "onboarding_nome":
        ...
        if not adv_id:
            await _zapi_client.enviar(phone, "❌ Esse e-mail já está cadastrado.\nDigite outro e-mail.")
...
async def _handle_conversa(phone, adv, texto):
    ...
    if not await db.pode_usar(adv["id"]):
        await _zapi_client.enviar(phone, "⚠️ Seu período de teste encerrou.\nAcesse *prazu.com.br* para assinar."); return
    if tl in ("prazos", "resumo", "briefing", "oi", "olá", "ola", "bom dia", "boa tarde", "boa noite"):
        ...
        await _zapi_client.enviar(phone, "🔍 Buscando publicações no DJEN...")
...
async def enviar_lembrete_trial() -> int:
    ...
    for adv in advogados:
        try:
            await _zapi_client.enviar(adv["whatsapp"],
